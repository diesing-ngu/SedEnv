...
```{r load_response}
resp <- read_sf(dsn = "input", layer = "SedEnv_4km_MaxCombArea_point")
...
plot(resp, pch = 20, cex = 0.75, col = "black", add = TRUE)
```
...
rm_resp <- cbind(resp$SedEnv, ov_resp)
names(rm_resp)[1] <- "SedEnv"
rm_resp <- rm_resp[-2]
...
```{r env_space_dist}
dist_env <- plot_geodist(resp, predictors,
                     type = "feature",
                     variables = sel_preds,
                     showPlot = FALSE)
...
v1 <- autofitVariogram(C1 ~ 1, as(resp, "Spatial"))
plot(v1)

#Class 5 (Erosjon/transport)
v5 <- autofitVariogram(C5 ~ 1, as(resp, "Spatial"))
plot(v5)

#Class 7 (Ingen/sakte avsetning)
v6 <- autofitVariogram(C7 ~ 1, as(resp, "Spatial"))
plot(v6)
...
```{r env_space_dist2}
dist_env2 <- plot_geodist(resp, predictors,
                     type = "feature",
                     cvfolds= index,
                     variables = sel_preds,
                     showPlot = FALSE)
